...
#include<time.h>
int main(){
	int soBiMat=0,soHienTai=0,i=0,dk,mode;
	const int Max=100,Min=1;
	srand(time(NULL));
//	soBiMat=(rand()%(Max-Min+1)+Min);
	printf("******MENU*****\n");
	printf("Xin chon che do choi 1 nguoi or 2 nguoi: ");scanf("%d",&mode);
	switch(mode){
		case 1:
			{
			soBiMat=(rand()%(Max-Min+1)+Min);
 do{
	do{
		printf("Moi ban nhap so doan: ");scanf("%d",&soHienTai);
...
do{
	do{
		...
		printf("\nMoi nguoi choi thu hai nhap so du doan: ");scanf("%d",&soHienTai);
		if(soBiMat>soHienTai){
